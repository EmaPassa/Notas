...
// IDs de las carpetas de Google Drive - ACTUALIZADOS CON NUEVAS UBICACIONES
const FOLDER_IDS = {
...
function testScript() {
  console.log("=== INICIANDO PRUEBAS CON NUEVOS IDs Y ÍNDICES CORRECTOS ===")
  ...
  try {
    ...
    console.log("4. Probando extracción de info del curso...")
      ...
      try {
        ...
        if (files.hasNext()) {
          ...
          if (file.getName().match(/\.(xlsx|xls)$/i)) {
            const cursoInfo = obtenerInfoCursoDelArchivo(file)
            console.log(`Archivo: ${file.getName()}, Curso: ${cursoInfo}`)
            return // Solo probar el primer archivo
          }
        }
      } catch (error) {
        console.error(`Error probando carpeta ${especialidad}:`, error)
      }
    })
  } catch (error) {
