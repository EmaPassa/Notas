// Google Apps Script para leer calificaciones desde Google Drive - VERSIÓN FUNCIONAL
    ...
    Object.keys(FOLDER_IDS).forEach((especialidad) => {
      try {
        ...
        while (files.hasNext()) {
          const file = files.next()
          const fileName = file.getName()
          const alumnosEncontrados = buscarAlumnoEnArchivo(file, nombreAlumno, fileName, filtroCuatrimestre)
